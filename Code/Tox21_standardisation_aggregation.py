...
dict_smi_name ={}
for smi,name in zip(df['standardised_smiles'],df['Molecule Name']):
    if smi not in dict_smi_name:
        dict_smi_name[smi] = [name]
    else:
        dict_smi_name[smi].append(name)


#drop columns
df_filtered.drop(labels=['source', 'Smiles','Molecule Name'],axis=1,inplace=True)
