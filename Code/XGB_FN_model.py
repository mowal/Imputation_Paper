# -*- coding: utf-8 -*-
"""
Created on Wed Mar  3 14:00:23 2021
...
for assay in assays:
    ...
    #get labels of other assays to form final X_train
    df_filled_assay_train = df_filled.drop(assay,axis=1).copy()
    
    #retain only labels for compounds that have an actual label for assay
    X_train_labels = df_filled_assay_train.iloc[dict_indices_train[assay],:].to_numpy(copy=True)
    
    #combine X_train_fp and X_train_labels
    X_train = np.concatenate([X_train_fp,X_train_labels],axis=1)
    ...
    #get labels of other assays to form final X_test
    df_filled_assay_test = df_filled.drop(assay,axis=1).copy()
    
    #retain only labels for compounds that have an actual label for assay
    X_test_labels = df_filled_assay_test.iloc[dict_indices_test[assay],:].to_numpy(copy=True)
    
    #combine X_train_fp and X_train_labels
    X_test = np.concatenate([X_test_fp,X_test_labels],axis=1)
